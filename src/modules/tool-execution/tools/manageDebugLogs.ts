...
export async function handleManageDebugLogs(conn: any, args: any) {
    try {
        // Validate inputs
        if (!args.username) {
      ...
      `);
        ...
        `);
                ...
                try {
                    ...
                    const deleteResults = await Promise.all(traceFlagIds.map((id: any) => conn.tooling.sobject('TraceFlag').delete(id)));
                    return {
                        content: [{
                                type: "text",
                    ...
                    try {
                        ...
                        const traceFlagIds = traceFlags.records.map((tf: any) => tf.Id);
                        const updateResults = await Promise.all(traceFlagIds.map((id: any) => conn.tooling.sobject('TraceFlag').update({
                            Id: id,

...
export async function grantFieldPermissions(conn: any, objectName: any, fieldName: any, profileNames: any) {
    try {
      ...
      SELECT Id, Name
      FROM Profile
      WHERE Name IN (${profileNames.map((name: any) => `'${name}'`).join(', ')})
    `);
...
export async function handleManageField(conn: any, args: any) {
                ...
                case 'MultiselectPicklist':
                    if (fieldProps.picklistValues) {
                        metadata.valueSet = {
                            valueSetDefinition: {
                                sorted: true,
                                value: fieldProps.picklistValues.map((val: any) => ({
                                    fullName: val.label,

...
export function buildWithClause(withClause: any): any {
...
export async function handleSearchAll(conn: any, args: any) {
    ...
    try {
        ...
        const returningClause = objects
            ...
            if (obj.where)
                clause += ` WHERE ${obj.where}`;
            if (obj.orderBy)
                clause += ` ORDER BY ${obj.orderBy}`;
            if (obj.limit)
                clause += ` LIMIT ${obj.limit}`;
            return clause + ')';
        })
            .join(', ');
      ...
      ${accessClause}`.trim();
        ...
        objects.forEach((obj: any, index: any) => {
            const objectResults = result.searchRecords.filter((record: any) => record.attributes.type === obj.name);
            formattedResults += `\n${obj.name} (${objectResults.length} records found):\n`;
            if (objectResults.length > 0) {
                objectResults.forEach((record: any, recordIndex: any) => {
                    ...
                    if (withClauses?.some((w: any) => w.type === "METADATA")) {
                        formattedResults += `    Metadata:\n      Last Modified: ${record.attributes.lastModifiedDate}\n`;
                    }
                    if (withClauses?.some((w: any) => w.type === "SNIPPET")) {
